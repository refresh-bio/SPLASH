...
public:
	...
	std::pair<T, T> min_max_coeff()	const
	{
		auto r = std::minmax_element(ptr, ptr + n_elems);
	...
	friend matrix_1d<T> operator-(const matrix_1d<T>& vec, const T x)
	{
		matrix_1d<T> res(vec);

		for (size_t i = 0; i < res.size(); ++i)
			res.ptr[i] = res.ptr[i] - x;
		return res;
	}
...
template<typename T>
matrix_1d<T> sign_01(const matrix_1d<T>& x)
{
	auto res(x);
