...
void process_kmc_db(const std::string& path, uint64_t sample_id, const Header& header, std::vector<buffered_binary_writer>& bins, std::vector<std::vector<uint8_t>>& buffers, uint64_t anchor_sample_counts_threshold, const PolyACGTFilter& poly_ACGT_filter, Stats& stats)
{
	CKMCFile kmc_db;
	...
	while (kmc_db.ReadNextKmer(kmer, count)) {
		...
		if (prev_anchor == anchor)
			...
			sort_merge_and_store(prev_anchor, targets_in_current_anchor, header, rec, bins, buffers, anchor_sample_counts_threshold, poly_ACGT_filter, stats);
...
int main(int argc, char** argv)
{
	...
	const size_t tot_buffer_size = 1ull << 30;
	const size_t single_buffer_size = tot_buffer_size / out_files.size();
	std::vector<std::vector<uint8_t>> buffers(out_files.size(), std::vector<uint8_t>{});
	for (auto& b : buffers)
		b.reserve(single_buffer_size);

	Stats stats;

	PolyACGTFilter poly_ACGT_filter(params.poly_ACGT_len);

	process_kmc_db(params.input_sample.input_kmc_db_path, params.input_sample.sample_id, header, out_files, buffers, params.anchor_sample_counts_threshold, poly_ACGT_filter, stats);
