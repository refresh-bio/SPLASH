...
class buffered_binary_reader {
	...
	size_t calc_buff_size(const std::string& path, size_t max_size) {
		FILE* f = fopen(path.c_str(), "rb");
		if (!f) {
			std::cerr << "Warning: cannot open file " << path << " to estimate buffer size\n";
			return max_size;
		}
		fseek(f, 0, SEEK_END);
		auto fsize = ftell(f);
...
public:
	buffered_binary_reader(const std::string& path, size_t buff_size = 1ull << 25) : buff(calc_buff_size(path,buff_size)) {
#ifdef USE_ZSTD_FOR_TEMPS
