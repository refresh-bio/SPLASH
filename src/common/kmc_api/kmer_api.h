...
public:
	...
	inline uint64 subkmer(uint32 pos, uint32 len) {
		...
		//auto end_pos = ((pos + byte_alignment) % 32) * 2;

		auto symbols_from_hi = 32 - (pos + byte_alignment) % 32;

		auto symbols_from_lo = len - symbols_from_hi;

		auto hi_part = kmer_data[hi_row] << (2 * symbols_from_lo);
		auto lo_part = kmer_data[lo_row] >> (64 - symbols_from_lo * 2);
