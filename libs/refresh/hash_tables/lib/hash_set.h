...
template<typename Key_t,
	...
	public:
		...
		bool check(const key_type& key, const size_t hash_val) const
		{
//			return _check_noinline(key, hash(key) & allocated_mask);
			return _check_noinline(key, hash_val & allocated_mask);
		}
		...
		size_type count(const key_type& key) const
		{
			...
			return (size_t)compare(data[pos], key);
		}

		size_type count(const key_type& key, const size_t hash_val) const
		{
			size_t pos = _find(key, hash_val & allocated_mask);
	...
	private:
		...
		size_t _find(const Key_t key, size_t pos) const
		{
		...
		bool _check(const Key_t key, size_t pos) const
		{
		...
		REFRESH_NO_INLINE bool _check_noinline(const Key_t key, size_t pos) const
		{
			if (compare(data[pos], empty_key))
				return false;
			if (compare(data[pos], key))
