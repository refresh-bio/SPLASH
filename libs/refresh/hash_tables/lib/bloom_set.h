...
private:
	...
	static size_t normalize_size(size_t size, double expected_fp_rate)
	{
		...
		size = std::max((uint64_t)256, size);
	...
	void insert_impl(const Key_t &x, const size_t h)
	{
	//	uint64_t h = mmh(x);
	...
	bool check_impl(const Key_t &x, const size_t h) const
	{
		size_t pos = (h & mask) >> mask_shift;
